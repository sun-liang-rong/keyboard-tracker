...
export const useStatsStore = defineStore('stats', () => {
  ...
  const todayStats = ref<TodayStat | null>(null)
  ...
  const activeHours = computed(() => {
    ...
    return hourlyDistribution.value.filter((count: number) => count > 0).length
  })

  // 专注时段：连续有按键的时间段数（简化：相邻小时有按键算一个时段）
  ...
  async function fetchTodayStats() {
    try {
      if (window.electronAPI) {
        ...
        // 获取今天日期，确保 selectedDate 也是今天（使用本地时区）
  ...
  function startListening() {
    ...
    if (window.electronAPI) {
      ...
      window.electronAPI.onKeystrokeUpdate((data) => {
        console.log('[Renderer] Keystroke update received:', data)
